...
NSString    *gender         = nil;
NSString    *color          = nil;
NSString    *marital_status = nil;
NSString    *custom_variable= nil;
int         age;
int         height;
int         weight;
      ...
      if([adtype isEqualToString:@"Video"])
    	{
             ...
             NSLog(@"Video ad displaying");
             ads = AdResponse;
        }
      else
       {
		
           NSLog(@"basic, mraid, iab ad displaying");
           ads = AdResponse;
...
- (void)getURLValues{
        ...
        //Custom Variable value pass
   
        NSString *location1     = [NSString stringWithFormat:@"&location=%@",location];
        NSString *macaddress1   = [NSString stringWithFormat:@"&macaddress=%@",macaddress];
        NSString *wlan1         = [NSString stringWithFormat:@"&wlan=%@",wlan];
        NSString *vlan1         = [NSString stringWithFormat:@"&vlan=%@",vlan];
        NSString *age1          = [NSString stringWithFormat:@"&age=%d",age];
        NSString *gender1       = [NSString stringWithFormat:@"&gender=%@",gender];
        NSString *color1        = [NSString stringWithFormat:@"&color=%@",color];
        NSString *height1       = [NSString stringWithFormat:@"&height=%d",height];
        NSString *weight1       = [NSString stringWithFormat:@"&weight=%d",weight];
        NSString *marital_status1= [NSString stringWithFormat:@"&marital_status=%@",marital_status];
        NSString *custom_var1   = [NSString stringWithFormat:@"&%@",custom_variable];
    
        //Appending URL with params
        NSString *urlString = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@&systemtype=ios",Loadurl,zone_id,adwidth,adheight,deviceType1,systemVersion1, systemName1, sizeheight1, sizewidth1,displaywidth1,displayheight1 ,carrierName1,ip1,userphone,lattitude1,longitude1,connectiontype1,udid1,timezone,useragent1,appid1,language1,sha1hashed1,md5hashed1,displaytype1,location1,macaddress1,wlan1,vlan1,age1,gender1,color1,height1,weight1,marital_status1,custom_var1];
    ...
    NSLog(@"URL- %@",encodedUrl);
        ...
        //Custom Variable Additional Work
          NSString *decode=[decode1 stringByReplacingOccurrencesOfString: @"'></script>" withString:[NSString stringWithFormat:@"&%@'></script>",custom_variable]];
...
- (void )getParameterValues {
	   ...
	   // uuid 		= [[NSUUID UUID] UUIDString];
	    ...
	    Loadurl         = @"http://adserver.elitecore.co.in:8080/api/request1.php?";
	   //Loadurl  		= @"http://192.168.1.200/development/shalin_dev/api/request1.php?";
